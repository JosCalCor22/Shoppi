...
function AsideCart (){
  ...
  return (
    <aside className={`${context.closeItem ? 'flex' : 'hidden'} flex-col sticky gap-5 top-5 w-[360px] h-fit border-2 border-[#351e1a] rounded-lg`}>
      ...
      <section className="px-5">
        ...
        <figure className="grid gap-2">
          <img className="w-full rounded-lg" src={context.showDetails.images[0]} alt={context.showDetails.title} />
          <figcaption className="text-sm text-justify px-2 font-slate-700">{context.showDetails.description}</figcaption>
        </figure>
        <div className="flex justify-between mt-5 mb-3 p-3">
          <div>
            <h2>Price: </h2>
            <p className="text-lg font-semibold">${context.showDetails.price}</p>
          </div>
          <button className="flex items-center gap-2 border-2 border-[#351e1a] px-3 py-2 rounded-lg">
            <span>Comprar</span>
            <FiShoppingCart />
          </button>
        </div>
